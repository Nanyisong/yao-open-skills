...
3. **校正理性假设**：输入真实历史行为数据和相关经验参考，修正对手理性概率和承诺兑现概率。
4. **建立反应模型**：输出对手反应地图、收益矩阵、可信承诺检查、信号质量和可能均衡。
5. **转成行动建议**：给出推荐动作、为什么不是其他动作、当前策略准备度、敏感性分析、下一步要补的信息和触发重算的条件。
...
实际跑的时候有个坑：AI 估算 payoff matrix 倾向于过度简化，尤其在多轮博弈的 credible commitment 判断里，模型容易高估对手的理性程度。

这个 Skill 因此新增了历史行为校正层。它会优先读取：

- 竞品过去是否真的持续降价
- 免费版发布后是否有真实产品、预算、渠道和交付投入
- 渠道攻击是否真正兑现过返点或独家政策
- 公开威胁是否经常反转
- 同类市场、同类谈判、同类渠道冲突中的经验参考

报告会输出每个关键玩家的“先验理性概率”和“历史校正后的理性概率”，并把它用于承诺可信度、敏感性分析和下一步信息缺口。

例如价格战样例里，头部竞品的理性概率会根据过往低价威胁反转、免费版投入不足等历史行为被下调，从而避免把长期低价威胁误判成完全可信承诺。
