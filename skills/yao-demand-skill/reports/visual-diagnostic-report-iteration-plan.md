...
- 保持纯白背景、稳定排版、表格/图表不溢出、HTML 顶部置顶菜单。
...
| 9 | JTBD 强度图 | 条形 / 矩阵 | 哪些任务最值得主攻？ | `segments.job_to_be_done` |
| 10 | 竞品与替代方案定位图 | 二维象限 | 为什么用户会从替代方案切换？ | `competitors` |
| 11 | 采用阻力漏斗 | 漏斗图 | 从知道到持续使用卡在哪里？ | adoption blockers |
| 12 | 证据质量分布 | 堆叠条 / 金字塔 | 结论靠事实还是假设？ | `evidence` |
| 13 | 风险矩阵 | 影响 x 概率矩阵 | 哪些风险最可能阻断需求？ | `risks` |
| 14 | 建议优先级矩阵 | Impact x Effort | 先做哪几个动作？ | `recommendations` |
| 15 | 预测情景图 | 三情景折线 / 区间图 | 修短板后结果可能怎样？ | `forecast` |
